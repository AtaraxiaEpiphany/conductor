...
**The fixed spine stays action-driven.** The shape adds a *diagnostic*, not a rewrite: the planner→executor→checker sequence is implicit in the dispatch actions, and the shape only *discloses* (via `shape_violation`) when an action drifts off the declared topology — it does not refuse or reorder it. This is the OpenSpec pattern (the walker owns topology; the agents own intelligence) applied at the altitude the codebase is already built for — explicitly NOT a model-authored harness, which would break the deterministic resumability the `step`/`wave` spines bought. Making the shape genuinely drive ordering (rather than diagnose drift) is a future code change at the dispatch emit site.
