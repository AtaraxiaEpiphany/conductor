...
- **Tag determines whether TDD is required** — resolved from the registry profile at dispatch (the `[Conductor Registry]` block `task-executor` receives carries the resolved `gates`/`grounding` for the locked task's leading tag). A tag whose profile carries a bespoke workflow — `workflow_doc` (a steps-library docfile; plugin `templates/workflow/steps/`, project `conductor/workflow/steps/` wins) or a legacy inline `workflow` string; a project may register either — diverges from default TDD: the executor's **per-dispatch manifest** (the `WORKFLOW_FILE` line in its envelope) pins the ONE resolved path decision (docfile | inline | fast-path | default TDD); the inline form is fetched with `track-state registry-doc --tag <Tag>` and followed verbatim. A both-exempt tag (whose `gates` omit `tdd` and `coverage`) takes the Step-8 fast-path the same manifest names. A tag whose profile carries `refactor: true` (today: `[Refactor]`) opts the task into the tactical refactorer at the §3.6c seam — the orchestrator dispatches `conductor:refactorer` once after the task succeeds, with no `[Refactor]` name marker or env required (those remain as per-task / per-session escape hatches for a task whose leading tag is something else). A tag whose profile carries an `agent` field (a rostered wrapper-agent name — `track-state roster add` generates the wrapper + its roster row) dispatches that wrapper as the task's executor **persona** instead of `task-executor` — the plan→skill seam: a skill adopted as a wrapper becomes the deterministic executor for a whole class of work, riding the same manifest/tripwire/retry scaffold (it occupies the task-executor SLOT; the binding must name a row in the merged agent roster, and runtime fail-opens to `task-executor`). Persona binding is **serial-rail-only**: a wave member always dispatches `task-executor` regardless of its class's binding. A project overlay may add more bespoke-workflow, refactor-opt-in, or persona-bound tags the same way — **the registry, not this file, is canonical for tag-specific executor behavior.**
...
- **Form:** the `workflow_shape` field on `track-state.json` is a bare shape name (e.g. `default`, `research-first`). `init-from-plan --shape <name>` writes the confirmed shape at state creation (validated against the resolved vocab; an unknown name hard-rejects the init — a deliberate declaration never silently no-ops); no flag → `default`.
