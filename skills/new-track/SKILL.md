...
**Tag-confirm relay (only when the block carries a `TAG_CONFIRM:` section).** The spec-planner authors each tag itself from the registry vocab (§4.2 of its body — planner judgment, no matcher round-trip) and lists there every authored tag that **drops the TDD and/or coverage gate** on its task. No `TAG_CONFIRM:` section → proceed (the gate-neutral/untagged common case). Section present → you own the human review loop, exactly like §2.4: **ONE `AskUserQuestion` round**, one question per listed task — "Keep `[<Tag>]` on `<task name>`?" with options **Keep `[<Tag>]`** (recommended — the planner authored it from the task's deliverable) vs **Drop to default TDD** (strip the tag; the task runs full gates). Apply each **Drop** inline via Edit on `<track_dir>/plan.md` — remove the leading `[<Tag>]` marker from that task line (a tag drop is a sanctioned minor inline fix, §2.4) — and announce the drops in one line. Keep a per-run ledger: if a later re-dispatch (this §2.3 regen loop or the §2.4 re-plan) re-tags a task the user already declined, drop it silently and note it in the announce; never re-ask a tag the user already kept.
...
Re-parse the returned `---SPEC PLAN RESULT---` block (halt on FAILURE), run the **tag-confirm relay** above on any `TAG_CONFIRM:` lines it carries (ledger applies — never re-ask already-decided tags), then loop back to step 1.
