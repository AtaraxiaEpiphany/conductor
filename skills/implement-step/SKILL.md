...
| `dispatch_phase_checker` | **Dispatch `conductor:phase-checker`**, prompt the fenced ``prompt`` field **verbatim** (pre-assembled from the fanned verdicts). After it returns, read its `---CHECKPOINT RESULT---`: `STATUS` (`PASSED`/`FAILED`), `CHECKPOINT_SHA` (if `PASSED`), `FAILURE_REASON` (if `FAILED`). Run **`track-state phase-checkpoint-review "<td>" --status <STATUS> [--sha <SHA>] [--reason "<R>"]`** — this owns the §3.7 stamp/route in code. `PASSED` → §2.0. `FAILED` → the spine owns the branch in code: if the output carries `routed_recovery: true` (an auto-routing track — `recovery_policy=auto` or continuous) → **§2.0** (the next `step` dispatches the phase failure-analyst, `dispatch_phase_failure_analyst`); otherwise (an ask-surface track) → announce the reason and **STOP** (byte-identical to before — re-invoke after editing spec/plan re-runs the phase). |
