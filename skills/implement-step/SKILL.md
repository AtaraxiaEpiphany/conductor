...
| `dispatch_batch` | **Fire `conductor:<member.agent>` for each entry in `wave`, in ONE message (parallel Agent calls)**, prompting each member's ``prompt`` field **verbatim** (pre-assembled — the verifier set is dynamic: `ac-tracer` always, plus either `test-runner` on a suite-gated phase OR `compile-runner` on a build-gated `compile`/`none` phase; the wave names exactly which ran). Then read the RESULT blocks and **transcribe** them to the spine: from ac-tracer's `---AC TRACE RESULT---` take `VERDICT` (+ `GATE` if `FAILED`, `N_UNGROUNDED` if `warn`). From the second verifier — whichever ran — take `STATUS` + `COMMAND`: if `test-runner` ran, read its `---L1 VERIFY RESULT---` and pass `--l1-status`/`--l1-command`; if `compile-runner` ran, read its `---BUILD VERIFY RESULT---` and pass `--build-status`/`--build-command`. Run **`track-state phase-verdict "<td>" --ac-verdict <V> [--ac-gate <G>] [--ac-n-ungrounded <N>] {--l1-status <S> --l1-command "<CMD>" | --build-status <S> --build-command "<CMD>"}`** — exactly one second-verifier ran, so exactly one of the L1/build pairs is present (this owns the §3.2 parse+assemble in code). Then → §2.0. |
...
| `halt` | Announce the `reason` + `reasoning` (+ `impact` / `evidence` / `recovery` when present) to the user → **STOP**. A deliberate stop-for-human (skip-analyze `pause_and_escalate` / `retry_with_modification`, a refute `SUSTAINED` overriding a skip, or failure-analyst `replan`/`escalate`). `decompose` does NOT halt — it arrives as an `ask` (see `dispatch_failure_analyst` above). When `recovery` is present it is the safe manual recipe — act on it (on `decompose`, preserve the original task's commit; do NOT revert), then re-invoke to resume. Otherwise edit the spec/plan/task per the reasoning, then re-invoke. |
