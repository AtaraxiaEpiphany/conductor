...
| `serial` | Run `track-state step "<td>"` ONCE and relay exactly what it emits (`dispatch` / `dispatch_batch` / `ask` / `done` — handle each per `implement-step` §2.0; a `dispatch_batch` fires the ac-tracer + second-verifier fan-out — `test-runner` or, on a build-gated `compile`/`none` phase, `compile-runner` — then `/conductor:implement` §3.2 synthesizes). Then → §2.0 — `wave-step` re-checks for waves (the serial task may have unlocked one). **`ineligible` is MANDATORY reading, not optional** — announce every entry's `reason` (`subtasked` / `non_executor` / `no_deps_comment` / `deps_unsatisfied`); a silent fall-to-serial reads as a bug. When the reasons are dominated by `no_deps_comment`, **lead with the actionable one-liner** before running serial: *"No task in this phase carries a `<!-- deps: -->` annotation — waves require explicit deps opt-in. To enable parallelism, annotate independent tasks in plan.md (e.g. `- [ ] Task  <!-- deps: P1.T1 -->`, or `<!-- deps: -->` for no deps); spec-planner authors these by default. Running serially for now."* This converts the conservative serial default (intentional — see `wave-flat-only-seam`) from a silent fall-back into an explained, fixable one. |
| `phase_checkpoint` | `/conductor:implement` §3.2 — fan out `ac-tracer` + the second verifier the wave names (`test-runner` or `compile-runner`) in parallel, then `phase-checker` synthesizes. Then → §2.0. |
