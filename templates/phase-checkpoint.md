...
3.  **Execute Automated Tests with Proactive Debugging:**
    -   **Phase-verify directive (check first):** if the current `## Phase N:` heading in `plan.md` carries a `<!-- verify: <modes> -->` directive, follow the phase-checker's Phase-verify directive branch instead of the test gate below — `compile` means run the **build** command from `dev-commands/<lang>.md` (NOT the tests), skip fix-and-retry, and checkpoint on a green build; `start` adds a one-shot app-boot smoke check. Absent directive → the full test gate below applies. **If this is an all-`[Migrate]` phase (every non-`[Manual]` task tagged `[Migrate]`) and the suite is red, a missing directive is almost certainly the friction** — a `[Migrate]` "bump deps" phase whose goal is "it compiles" should carry `<!-- verify: compile -->` (the suite is *expected* red there); a "fix failing tests" phase should carry `<!-- verify: test -->`. The phase-checker's migration-phase branch will report `STATUS: FAILED` and prescribe the exact directive to add, so this is self-correcting on the first checkpoint.
