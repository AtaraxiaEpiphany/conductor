...
The `bin/track-state` command provides direct state management. Run `bin/track-state help` for the full, current list (88 subcommands across 17 groups) — it is grouped and self-describing. The complete surface, straight from `track_state/commands.py` (the same single source the pre-command guard derives its sanctioned set from):
