...
form) — NOT a hand-maintained tag/mode vocabulary. The tag/mode **vocabulary +
semantics** live in the resolved registries (``task-type-profiles.json``,
``verify-mode-profiles.json`` — plugin baseline ⊕ project overlay), rendered by
``track-state registry-doc``.

A hand-maintained tag/mode enumeration *table* in the contract would be a third
home for the vocabulary (alongside the registry and the ``[Conductor Registry]``
block injected into agents) and the first to drift: a project overlay adds a tag
or mode and the contract silently contradicts it. This script makes "never drift"
a CI guarantee, not a hope — the same discipline as ``check-plan-annotations``.
...
``TAG_VOCAB``) or a known mode literal (``compile``, ``anchor``, … — every entry
in the resolved ``MODE_VOCAB``). Tag literals are matched as the bracketed form
``[<Tag>]`` because that is how they appear in plan.md / the contract; modes are
matched bare (``compile``, ``test``, ``start``, ``anchor``) as backticked or raw
first-cell tokens.

What it does NOT flag
---------------------
Prose mentions of a tag/mode that are NOT a table row — e.g. a Rule keyed on a
specific tag (``Tag it with [Manual] …``), a grammar example
(``- [ ] [Migrate] bump spring-boot``), or the ``<!-- verify: compile -->``
directive examples. Those are legitimate: they are grammar/invariant text, not a
vocab enumeration. Only a tabular enumeration (the data-duplication shape) trips
the gate.
...
def main():
    ...
    if findings:
        sys.exit(
            ...
            "table.\nA tag/mode enumeration in the contract is a third home for the "
            "vocabulary (registry + injected [Conductor Registry] block + this table) "
            "and the first to drift — a project overlay adding a tag/mode would silently "
            "contradict it.\nRemove the table; the resolved vocab is rendered by "
            "`track-state registry-doc` (full tables) / `--tag <Name>` / `--mode <name>`, "
            "and the [Conductor Registry] block injected into agents is authoritative at "
            "dispatch.\nDuplicated rows:\n" + "\n".join(findings)
        )

    print("OK: plan-format-contract.md carries no hand-maintained tag/mode enumeration "
          "table (vocab is registry-sourced).")
