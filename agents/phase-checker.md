...
2. Emit and follow that mode's `protocol` prose verbatim — it is the prompt-shaping instruction for executing this mode (it tells you exactly which command to run, the pass/fail conditions, the record line, and the FAILURE_REASON shape). Do NOT re-derive a mode's behavior here: the registry is the single source, so each mode's `protocol` describes its own gate; your job is to read and run it, not to keep a parallel per-mode description in this agent (a restated ladder is the first thing to drift when a mode is added or a project overlay redefines one).
...
The result block below is **report-field-driven, not mode-name-baked.** Each verify-mode declares a `report_field` in the registry — resolve it from the mode's row in your injected `[Conductor Registry]` block (each row is `mode: runs -> report_field`); do not restate the mapping here. Emit one result line per **declared** mode's `report_field` — the value grammar below (`<passed|failed|…>`) is what the result-block parser keys on; the *field name* is whatever the mode declared. A project-overlay mode with `report_field: LINT` flows through with zero prose edits here.
