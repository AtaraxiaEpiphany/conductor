...
- **Exploration-shaped work with NO code deliverable** (tagged or not) → **ERROR**: report **FAILURE** and include the exact token `MISROUTE: exploration work dispatched to task-executor` in the failure report. You know this case by the deliverable, not the tag: the task asks for findings, understanding, a map, or a survey — an answer, not a change to the codebase. Do not execute it as a code task; a wrong-tag misroute is recovered by the failure-analyst reroute verdict (it re-tags the task `[Explore]` and re-dispatches the explorer), so your refusal is the cheapest point of detection.
