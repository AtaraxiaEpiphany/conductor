...
2. **Tag-derived default.** Else compute the union of each top-level task tag's `default_verify` (the `[Conductor Registry]` block each tag carries — today only `[Migrate]` proposes one, `compile`; a project-overlay tag may propose its own). If the tags **agree** on a non-empty set → emit that directive (`verify: compile`). If they **conflict** (different tags propose different modes) → fall through; a mixed-type phase has no single gate, so the full gate is correct.
3. **Goal-derived default.** Else classify the phase goal/name with the same heuristic `derive_verify_modes` encodes: a goal whose words mean "it compiles" (mid-migration intermediate) → `verify: compile`; a goal whose words mean "the app boots / starts" (final integration) → `verify: test,start`; a refactor goal with a frozen anchor → `verify: anchor`. Non-empty → emit that.
4. **Full gate.** Else emit nothing — the default full gate is correct for feature work (the majority of phases).

This is a **proposal**, not authoring: `init-from-plan --check` warns (never blocks) on the result, and the operator may override any phase by editing its heading. The canonical migration plan is what this procedure *produces*: every intermediate migration phase (`[Migrate]` tasks, "compiles" goal) resolves to `verify: compile`; the final integration phase ("app boots" goal) resolves to `verify: test,start`. Author the directive by hand only when you want a result the procedure would not reach (e.g. `verify: anchor` on a specific refactor phase, or `verify: test,adversarial` on a security phase).
