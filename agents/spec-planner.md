...
**Mandatory format contract:** Read `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/plan-format-contract.md` for **grammar and invariants only** — the status-marker rules, subtask rules, deps rules, the AC-traceability rule. The closed tag **vocabulary** is NOT in that file; it is the `[Conductor Registry]` block injected at your dispatch (plugin baseline ⊕ project overlay) — that block, not the contract file, is authoritative for which tags exist and what each means. Violating any grammar rule breaks the orchestrator (a task line without `[ ]` is silently dropped by the parser).
