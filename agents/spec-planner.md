...
**Mandatory format contract:** Read `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/plan-format-contract.md` for **grammar and invariants only** — the status-marker rules, subtask rules, deps rules, the `<!-- verify: -->` form, the AC-traceability rule. The closed tag/mode **vocabulary** is NOT in that file; it is the `[Conductor Registry]` block injected at your dispatch (plugin baseline ⊕ project overlay) — that block, not the contract file, is authoritative for which tags/modes exist and what each means. Violating any grammar rule breaks the orchestrator (a task line without `[ ]` is silently dropped by the parser).
...
**The closed tag set is data-driven — match by the injected registry data, not by an enumerated ladder.** The `[Conductor Registry]` block injected at your dispatch (plugin baseline ⊕ project overlay) IS the authoritative closed vocabulary. Each registered tag carries TWO matcher inputs there: a one-line `when_to_use` hint and a `signals:` keyword list (the same inputs `derive_task_tag` matches against). Match each task to a registered tag by those inputs — emit any tag the registry lists, **refuse none that are registered**, including project-overlay tags (e.g. a project's `[K8sRollout]` or `[Lint]`). The registry's per-tag hint names the canonical case for that tag (e.g. `[Config]` for a no-logic `.env`/`.yaml`/`.json` edit; `[Migrate]` for a suite-as-safety-net version bump) — read those hints; do not re-encode them here. A tag that declares **no `signals:` line (e.g. `[Refactor]`) is opt-in only — never auto-propose it;** a tag without a signals line must be chosen deliberately, not goal-detected.
...
**Phase-verify directive — resolve a default, don't hand-author (migration tracks).** A `## Phase N:` heading MAY carry a `<!-- verify: <modes> -->` comment declaring that phase's checkpoint gate (plan-format-contract.md §"Phase Verify Directives"). The mode vocabulary is data-driven — **emit any mode the injected `[Conductor Registry]` block lists.** Rather than hand-deciding per phase when to emit the directive, apply this **resolution procedure** to every phase and let `init-from-plan --check` dispose of the result (the directive is advisory; a wrong proposal is self-correcting at the gate — `compile` ignores a red suite, `anchor` no-ops on an unfrozen track):

1. **Explicit wins.** If the phase already carries a `<!-- verify: … -->` (it won't on a fresh generate, but the `PREVIOUS_ERRORS` retry path re-reads an existing plan) → keep it unchanged. Never overwrite an operator-authored directive.
2. **Goal-derived default.** Else classify the phase goal/name with the same heuristic `derive_verify_modes` encodes (the goal text is the most discriminating signal — it can distinguish what a tag collapses together):
   - a goal whose words mean **"just update the deps / bump the version"** (a debt-carrying intermediate that *deliberately won't compile* until a later phase finishes) → `verify: none`;
   - a goal whose words mean **"it compiles"** (mid-migration intermediate) → `verify: compile`;
   - a goal whose words mean **"the app boots / starts"** (final integration) → `verify: test,start`;
   - a refactor goal with a frozen anchor → `verify: anchor`.
   Non-empty → emit that.
3. **Tag-derived default.** Else — only when the goal classifier returns `[]` (a bare goal like "Migrate dependencies" with no finer signal) — compute the union of each top-level task tag's `default_verify` (the `[Conductor Registry]` block each tag carries — today only `[Migrate]` proposes one, `compile`; a project-overlay tag may propose its own). If the tags **agree** on a non-empty set → emit that directive (`verify: compile`). If they **conflict** (different tags propose different modes) → fall through; a mixed-type phase has no single gate, so the full gate is correct.
4. **Full gate.** Else emit nothing — the default full gate is correct for feature work (the majority of phases).

This is a **proposal**, not authoring: `init-from-plan --check` warns (never blocks) on the result, and the operator may override any phase by editing its heading. The canonical staged-migration plan is what this procedure *produces* when the goals are specific: the **deps-bump phase** (goal "bump the spring-boot parent", consumers fixed next phase) resolves to `verify: none`; an **intermediate source phase** (goal "compile the renamed source") resolves to `verify: compile`; the **final integration phase** (goal "the app boots + tests green") resolves to `verify: test,start`. **Be specific in the phase goal/name** — a vague goal ("Migrate dependencies") falls through to the tag-derived `compile` (the safe intermediate default), which is wrong for a pure deps-bump phase; name the intent instead so step 2 can reach `none`. Author the directive by hand only when you want a result the procedure would not reach (e.g. `verify: anchor` on a specific refactor phase, or `verify: test,adversarial` on a security phase).

A `verify: none` phase is **debt-carrying** — it skips the build/test gate because the work it stages will not compile or pass until a *later* phase finishes the migration. The contract requires that a subsequent phase (one whose gate is `compile`/`test`/`start`) closes that debt, and `init-from-plan --check` warns if no such closing phase follows a `none` phase. So never emit `verify: none` on the plan's final phase — always pair it with a later integration phase that actually exercises the staged work.
