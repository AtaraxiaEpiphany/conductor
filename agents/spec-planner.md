...
| `RESEARCH_NOTES`    | Optional. Path to pre-planning Exploration Notes — a shape Prelude's single explorer, or a §2.2.5 grounding fan-out's multiple slices as **semicolon-joined paths** (read EACH). When not `N/A`: read them FIRST, before the codebase scan, as primary context alongside the Brief — the shape's planning docfile (`track-state registry-doc --shape <WORKFLOW_SHAPE>` renders it) carries the full planning procedure for the shape. |
...
**Exploration Notes (if present).** If `RESEARCH_NOTES` is not `N/A`, read it/them FIRST — before the codebase scan — as primary context alongside the Brief: it is the pre-planning exploration map (a shape's Prelude ran `conductor:explorer` before you were dispatched; a grounding fan-out ran three sliced ones — the value is a semicolon-joined list of paths, one per slice: architecture/data-flow, api/contracts, tests/constraints/history — read EACH; together they are the enumerated ground your plan should name concrete files from). Anchor the plan in the structure the notes found; the shape's planning docfile (`PLAY_FILE`, §4.0) carries the full planner-facing procedure.
...
1. **Task-Type Vocabulary (fetch FIRST)** — Run `track-state registry-doc` (no args) and load the resolved task-type tag tables AND the `## Tag Signals` keywords BEFORE authoring any task line. This is the closed vocabulary your `plan.md` tags validate against (plugin baseline ⊕ project overlay). `init-from-plan` rejects an unrecognized tag as a hard error, so author tags ONLY from what this renders — never invent one. The registry is the single home for the tag set; you fetch it on demand rather than receiving it injected (the three-tier discipline — `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/context-model.md`). The `## Tag Signals` keywords are guidance for your §4.2 labeling judgment — classify by the task's deliverable, using signals as tiebreakers, not as the decider. Fetch the vocabulary here; author labels there.
...
**The closed tag set is data-driven — YOU author the label, the registry owns the vocabulary.** The `track-state registry-doc` render (plugin baseline ⊕ project overlay) IS the authoritative closed vocabulary (fetched in §3.1). For **every top-level task line, you choose its tag yourself**, using each row's `when_to_use` and `signals` as *guidance for your judgment* — you wrote the task, you know what it delivers; no command decides this for you. Write the chosen tag as the task's leading `[Tag]` marker, or leave the task **UNTAGGED when the deliverable is ordinary implementation** (untagged is the default full TDD path — a deliberate choice, not an omission). You MAY resolve ambiguity the signal tables cannot: an exploration task whose prose never uses a signal keyword still gets `[Explore]` — classify by the task's deliverable (findings/understanding → `[Explore]`; human action → `[Manual]`; no-logic config → `[Config]`), not by keyword presence. A tag that declares **no `signals:` line (e.g. `[Refactor]`) is opt-in only** — author such tags deliberately, from your knowledge of the work, never from description text. Emit any tag the registry lists, **refuse none that are registered**, including project-overlay tags (e.g. a project's `[K8sRollout]` or `[Lint]`). When a tag you author **drops the TDD and/or coverage gate** (`tdd_exempt`/`coverage_exempt` on its row), you MUST list it under `TAG_CONFIRM:` in §5.0 (the orchestrator relays one keep-vs-drop question to the user; you never drop it yourself). The registry's per-tag hint names the canonical case for that tag — read those hints; do not re-encode them here. `init-from-plan --check` will print an advisory when your declared tag disagrees with what the registry signals would have suggested — treat a disagreement as a prompt to double-check your judgment, not as an override of it.

**When unsure between an exemption tag and no-tag — leave the task UNTAGGED.** The default TDD path is the safe failure mode: a wrongly-untagged `[Config]` task costs one extra Red cycle, but a wrongly-tagged feature task silently skips TDD and the coverage gate (F2/F3 exempt). Defaulting to no-tag biases toward correctness. Never invent a tag **not in the resolved registry you fetched in §3.1** (e.g. `[Feature]`, `[Bugfix]`, `[Test]`, `[TDD]`) — `init-from-plan` **rejects** an unrecognized tag as a hard error (it validates against the resolved registry vocab), so an invented tag blocks the track from starting. The registry lives at `conductor/workflow/task-type-profiles.json` (baseline ⊕ project overlay — see `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/plan-format-contract.md`), and the project's overlay there is where a project-specific tag is registered so it appears in your `registry-doc` render.

**The fog test (complexity rule) — a SEPARATE question from the exemption one.** "When unsure → untagged" (above) is the right fail-safe for *exemption* tags, but it wrongly silences `[Explore]`: unsure about **which exemption applies** means default TDD; unsure about **the ground** means exploration. For each area the plan touches, ask: **can I name the concrete files/modules involved** — from the Exploration Notes (`RESEARCH_NOTES`), the Brief, or my own scan? Where you cannot, insert an `[Explore]` task ahead of the building tasks for that area (classify by deliverable, not by signal keywords — an exploration whose prose never says "explore" still gets `[Explore]`). The asymmetry licenses the split: a wrongly-added explore task costs one cheap explorer pass whose findings still feed the bridge (and a misroute is recoverable by design); a plan built on unexplored ground costs the whole track. Grounding notes present (a fan-out ran) make this test answerable — use them; notes absent makes the test MORE likely to fire, not less.
...
`TAG_CONFIRM:` is optional — include it ONLY when at least one authored tag drops the TDD and/or coverage gate (`tdd_exempt`/`coverage_exempt` on its registry row, §4.2), one line per such task exactly as shown; omit the section entirely when none. The orchestrator relays these as one keep-vs-drop question round and applies the drops.
