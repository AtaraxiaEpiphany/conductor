...
`TAG_CONFIRM:` is optional — include it ONLY when at least one authored tag drops the TDD and/or coverage gate (`tdd`/`coverage` absent from its registry row's `gates`, §4.2), one line per such task exactly as shown; omit the section entirely when none. The orchestrator relays these as one keep-vs-drop question round and applies the drops.
