...
- **SUBTASK dispatch pipeline**: Orchestrator now passes `SUBTASK` to task-executor dispatch prompt; task-executor writes subtask index to `result.json` instead of hardcoded `null`. Previously, `process-result` received `subtask=null` and called `_do_complete(s=None)`, which targeted the parent task and auto-completed all sibling subtasks whenever any single subtask finished ([afdf535](https://github.com/anthropics/conductor-plugin/commit/afdf535))
- **Phase-checker checkpoint**: Agent now uses full path `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/track-state" add-checkpoint` instead of bare `track-state` (not on PATH). Template updated from manual plan.md editing to CLI command. Dispatch prompt fixed to pass `TRACK_ID` and `PHASE_INDEX` (was `PHASE`) ([afdf535](https://github.com/anthropics/conductor-plugin/commit/afdf535))
- **Deferred marker regex**: `_do_sync_plan` and `_validate_plan_consistency` regex now matches `d` (deferred) marker — previously deferred tasks in plan.md were skipped during sync, causing stale markers on re-sync ([afdf535](https://github.com/anthropics/conductor-plugin/commit/afdf535))
