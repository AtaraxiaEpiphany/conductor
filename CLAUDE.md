...
For tasks tagged `[Explore]` (read-only code investigation), the orchestrator MUST dispatch the `conductor-explorer` subagent instead of `conductor-task-executor`. The explorer agent performs the investigation and documents findings to `{TRACK_DIR}/exploration.md`.
